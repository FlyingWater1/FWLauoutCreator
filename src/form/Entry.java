...
public class Entry extends JPanel {
    ...
    protected JCheckBox mCheckOnClick;
    ...
    public Entry(EntryList parent, Element element) {
        ...
        mID.setPreferredSize(new Dimension(140, 26));

        mCheckOnClick = new JCheckBox();
        mCheckOnClick.setPreferredSize(new Dimension(60,26));
        if (!element.id.contains(element.getFullID())) {
            mCheckOnClick.setSelected(mElement.used);
        } else {
            mCheckOnClick.setSelected(false);
        }
        mCheckOnClick.addChangeListener(new OnClickCheckListener(element));
    ...
    private void checkState() {
        if (mCheck.isSelected()) {
            ...
            mCheckOnClick.setEnabled(true);
        } else {
            mType.setEnabled(false);
            mID.setEnabled(false);
            mName.setEnabled(false);
            mCheckOnClick.setEnabled(false);
        }
    }
    ...
    public class OnClickCheckListener implements ChangeListener {

        private Element element;
        public OnClickCheckListener(Element element) {
            this.element = element;
        }

        @Override
        public void stateChanged(ChangeEvent event) {
            element.setClickable();
        }
    }
